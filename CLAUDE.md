...
# Ingest raw datasets into the standard REATS split (classification, for Module B)
...
# Add --detection to also write the YOLO-format split for Module A
# (data/detection/{split}/images/*.jpg + labels/*.txt — a separate subtree
# from data/{split}/{class}/, see "Ingestion pipeline" below for why), or
# call IngestPipeline.run_detection() directly for just that pass
...
**Module A** is pure PyTorch — no ultralytics at runtime. The `ultralytics` line in `requirements.txt` is only used by `verify_env.py`. `IRDetector.detect()` runs the full YOLOv4 forward pass + NMS internally. Training uses `MosaicDataset` which expects YOLO-format layout: `{data_root}/{split}/images/*.jpg` + `{data_root}/{split}/labels/*.txt` (class cx cy w h, normalised) — `data_root` is whatever path is passed to `MosaicDataset`/`IRDetector.train()`; the notebook passes `data/detection/` (see "Ingestion pipeline" below — **never** `data/` itself, which is Module B's classification root and would crash `ImageFolder`). Default checkpoint: `checkpoints/detector_bootstrap.pt`. `IRDetector(weights=path)` raises `FileNotFoundError` if the path doesn't exist; only `weights=None` gives (intentionally) random init. Dashboard `load_pipeline()` likewise raises on any missing checkpoint — no silent random-weight fallback.
...
**Threshold-operational policy** (`modules/threat_policy.py`): `map_confidence_to_policy(confidence, threat_level)` starts a detection at `NONE` and steps it up to `WARNING`/`TRACK`/`ENGAGEMENT` once confidence clears 0.50/0.75/0.90 respectively, then caps the result at the class's `threat_level` ceiling (RED→ENGAGEMENT, ORANGE→TRACK, YELLOW→WARNING) — so an ORANGE/YELLOW target can never reach ENGAGEMENT regardless of confidence. These numbers live in `config/targets.yaml`'s `operational_policy` section (loaded as `config.OPERATIONAL_POLICY`), not hardcoded in the module — treat the YAML as ground truth if this ever reads stale.
...
- `pipeline.py` — orchestrates parsers + label resolution + `preprocessor.py` patch extraction + stratified train/val/test split. `_resolve_label` returns `(class_id, matched)`; the run prints an **UNMAPPED** report per dataset listing raw labels with no map entry (the fix-list for `label_maps.yaml`). Writes `data/provenance.json`. `IngestPipeline.run()` writes **classification** crops only — one cropped patch per annotation under `data/{split}/{class}/` — for Module B. `IngestPipeline.run_detection()` is a separate pass over the same annotations for Module A: it groups by *source image* instead of by class and writes the opposite layout, `data/detection/{split}/images/*.jpg` + `data/detection/{split}/labels/*.txt` (YOLO `class cx cy w h`, normalised) — full frames with every box on them, consumed by `MosaicDataset`/`IRDetector.train()`. **Deliberately a separate `data/detection/` subtree, not `data/{split}/images` directly**: `build_loaders()`'s `ImageFolder` (Module B's classification loader) treats every subdirectory of `data/{split}/` as a class name, so writing `images/`/`labels/` folders there adds two bogus "classes" and hard-crashes the instant it scans `labels/` and finds zero image-extension files in it (`FileNotFoundError: Found no valid file for the classes labels` — `labels` there is a literal class name, not a description; this broke a real Kaggle run before the subtree split was added). As defense in depth on top of the subtree split, `build_loaders()` also uses `_REATSImageFolder`, a small `ImageFolder` subclass that overrides `find_classes()` to only accept directories whose name is an actual REATS class — so any future stray non-class directory under `data/{split}/` is ignored instead of crashing or silently becoming a bogus 44th class. Annotations with `bbox=None` (folder/video-folder datasets that only carry a class label, no real localisation) are skipped by `run_detection()` — labelling the whole frame as one box would be wrong wherever the target doesn't fill it — so a dataset can contribute to Module B's classification split while contributing zero images to Module A's detection split. `run_detection()` is resume-safe like `run()`: it derives each image's output filename deterministically (dataset key + source stem + frame index, plus a short hash of the full un-sanitised key to guard against two different sources sanitising to the same stem) and skips anything already written under any split — a stem only counts as "already written" once *both* its image and label file exist, so a write interrupted mid-pair (Kaggle timeout/OOM) self-heals on the next call instead of leaving a silently-unlabelled image forever. It does not reshuffle `self.rng` (shared, stateful, with `run()`) across already-written images, which would otherwise risk moving one from train to val/test on a later call and leaking the same physical image across splits. `_collect_by_class()`'s parsed-and-label-resolved annotations are cached on the `IngestPipeline` instance after the first call, so `run()` and `run_detection()` — which the notebook runs back-to-back on the same `pipe` object — don't each redundantly re-parse every dataset from scratch. CLI: `python -m ingestion.pipeline ... --detection` runs both passes.
...
The repo root also has `real-time-ex-03.ipynb` — an **executed** ~6MB export of the 2026-07-04 GPU run (22 code cells with saved outputs, `papermill` metadata), kept only as the provenance record for the 93.12%-accuracy result in `MEMORY.md`'s "Kaggle run results". It is a point-in-time snapshot, not the development notebook, and has already diverged (31 cells vs. this one's current 37) — always edit `REATS/notebooks/01_kaggle_full_pipeline.ipynb`, never the root file.

Run cells in order: `c-gpu` → `c-install` → `c-clone` → `c-config` → `c-ingest` → `c-ingest-detection` → `c-module-b` (or `c-train-ensemble`) → `c-faithfulness` → (Section 5) `971d98df` bootstrap → `c-train-detector` → `c-module-a` demo → `c-dashboard`. `c-ingest-detection` writes Module A's YOLO-format split right after `c-ingest`; `c-train-detector` (Section 5, after the Module A bootstrap cell) actually trains it, warm-started from the bootstrap checkpoint — both are no-ops (clearly logged, not silent) if no dataset in `DATASET_INPUTS` has real bounding-box annotations yet.
