...
## Kaggle run report compiled — 93.12% (07-04) is the reported figure; fast-mode run demoted to benchmark-only (2026-07-09)

A second real GPU run (fast-training mode, 75 epochs — added to the codebase 2026-07-08, so this run postdates that commit) was reviewed from its own Kaggle notebook logs and, in a follow-up pass, its five generated images (class distribution, confusion matrix, Grad-CAM by domain, Module A demo, A→B→C pipeline demo) directly. Written up in newly-committed `docs/kaggle_run_report.md`, alongside a reconstruction of the 2026-07-04 `real-time-ex-03` run below (whose own standalone report was never committed — exactly the "lost report" failure mode this entry exists to avoid repeating a second time).

**User decision: 93.12% (the 07-04 full 300-epoch run, below) is the figure to report/present. The fast-mode run's 95.50% is explicitly excluded from reporting and kept only as a benchmark** — `FAST_TRAINING_GUIDE.md` documents fast mode's own expected trade as a 1-2 point *drop* from full training, not a gain, so a higher measured number is a sign of data-composition inflation (33/43 classes still synthetic-only, synthetic-only accuracy ~98% vs. 87% real-backed), not better modeling. Full reasoning in the report's §3. Headline benchmark numbers, single ConvNeXt-tiny, 75 epochs / 53.7 min — **not for reporting**:

| Metric | Target | Result |
|---|---|---|
| Accuracy (test) | ≥92% | 0.9550 tabular / **0.953** as labeled on its own confusion-matrix image (a genuine ~0.2-point discrepancy, unexplained, flagged not resolved) — **excluded from reporting**, see decision above |
...
| Detector mAP@0.5 (separate 20-epoch fast-mode attempt) | ≥75% | 0.0003→0.0011 — effectively untrained; the demo image shows ~2,687 boxes blanketing a trivial 2-object test frame edge-to-edge, output uncorrelated with scene content, not just "inaccurate" |

Per-domain: AIR 97.8%, GROUND 95.3% (up sharply from 07-04's 85.23% — cause unconfirmed), **NAVAL 86.7%** (down sharply from 07-04's 99.75% — now the weak domain). The confusion-matrix image's single most visually prominent off-diagonal block is the Corvette/Destroyer/Frigate naval trio — direct visual confirmation of this finding, not just secondhand description. Grad-CAM-by-domain image shows AH64/BM21 (flat iconographic synthetic renders) at 3/3 correct with tight attention, vs. Corvette (textured silhouette renders) at 1/3 correct, 2 misclassified as Frigate — concrete evidence that iconographic synthetic classes inflate the headline number while realistic-looking naval renders carry the true difficulty. Plausible explanation (still unverified, needs a per-run domain-provenance breakdown): the 07-06 ingestion fixes added harder *real* naval images into a domain that was previously almost entirely synthetic-and-easy. Dashboard launched and ran end-to-end successfully with a public tunnel. Real-image-backed classes now 10/43 (up from 8/43), but this run's own UNMAPPED report reportedly still listed `SARScope_Maritime`, which conflicts with the 2026-07-06 "Ingestion parser fixes" entry below marking it fixed — possibly a stale `c-clone` (same class of issue as "Crash 1" below), unconfirmed; worth checking which commit this run actually pulled next time it's repeated.
