...
**Deferred / dead-ends (flagged to user, not fixed this pass):**
- `HRSC2016` (`guofeng/hrsc2016`) — messy mirror: real data is under `hrsc2016_dataset/` + `HRSC2016.part01..05/`, nested deeper than the depth-3 probe showed; the only XML the probe surfaced was an `AnnotationTool_v2/.../bin/Debug/` sample. Needs a deeper probe or switch to the `weiming97/hrsc2016-ms-dataset` fallback mirror. HRSC XML is native `<HRSC_Image>`/`<HRSC_Object><Class_ID>` with `.bmp` images — `parse_xml` already handles that shape, so it's purely a path problem.
- `SWIM` — VOC-style but **rotated boxes** (`<robndbox><cx><cy><w><h><angle>`); `parse_xml` only reads axis-aligned `xmin/ymin/...`, so it finds the `<name>wake</name>` but no coords → 0. Needs robndbox → AABB support (cx±w/2, cy±h/2). Medium value (wake/ship → naval).
- `Ships_Satellite` (`rhammell/ships-in-satellite-imagery`) — not detection: `shipsnet.json` pixel arrays + `shipsnet/shipsnet/*.png` 80×80 tiles whose **filename prefix** is the label (`1__…`=ship / `0__…`=not). Needs a bespoke filename-prefix loader. Low value (tiny optical tiles).
- `Aerial_Segmentation` — semantic-seg land-cover only; `classes.json` = Water/Land/Road/Building/Vegetation — **no military/vehicle/ship classes at all**, so the existing `vehicle/airplane/ship` map entry can never match. Dead end; leave at 0 (harmless).
- `Battle_Tank_UAV` substitute (`/kaggle/input/notebooks/awaisalisaduzai/tank-detection-vit`) — **junk**: the mount holds only `__results___files/*.png` and `__output__.json` (a kernel's stderr log), no tank dataset. Same class of error as `Aerial_Vehicle_Detection` — flagged to user for a real source; path left as-is pending their answer.

Naval reality check: even with SARScope + Thermal_Ships + Ships_Vessels_Aerial, real data still only feeds the naval size-rule classes (Destroyer/Frigate/Corvette/FastAttack) and a couple GROUND ones — the specific jets/tanks remain synthetic-only.
