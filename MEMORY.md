...
- **Still blocking real data from the 5 new keys:** even the 4 that now resolve get `not found in label_maps.yaml — skipping` (0 patches) until they get at least a stub `ingestion/label_maps.yaml` entry — the pipeline skips a dataset entirely before parsing if it has no map entry, so you can't get an UNMAPPED report for a new key without first adding a stub. Separately, several datasets that DO have entries reported UNMAPPED raw labels that are wrapper-directory names (`allimages`, `images`, `jpegimages`, `label`/`image`, `masks`) — the folder parser is landing on a media-less wrapper dir for these, worth a look alongside the label-map work
